...
import { API_KEY, APP_ID, APP_INFO, APP_INPUT_PARAMS_MODE_KEY, APP_MODE_OPTIONS, APP_SHOW_MODE_SWITCH, isShowPrompt, promptTemplate } from '@/config'
...
const Main: FC<IMainProps> = () => {
  ...
  */
  ...
  useEffect(() => {
    ...
    inputs[APP_INPUT_PARAMS_MODE_KEY] = inputs[APP_INPUT_PARAMS_MODE_KEY] || APP_MODE_OPTIONS?.[0].value
  ...
  const handleConversationSwitch = () => {
    if (!inited)
      return
    const item1 = localStorage.getItem('inputs') || '{}'
    // 默认直接开始对话
    handleStartChat(JSON.parse(item1))
  ...
  */
  ...
  const checkCanSend = () => {
    ...
    const emptyInput = (APP_SHOW_MODE_SWITCH ? inputLens < (promptVariablesLens - 1) : inputLens < promptVariablesLens) || Object.values(currInputs).find(v => !v)
    if (emptyInput) {
  ...
  const handleSend = async (message: string, files?: VisionFile[]) => {
    ...
    let paramsInputs = currInputs
    if (APP_SHOW_MODE_SWITCH) {
      paramsInputs = {
        ...currInputs,
        [APP_INPUT_PARAMS_MODE_KEY]: localInputs?.[APP_INPUT_PARAMS_MODE_KEY],
      }
    }
    ...
    sendChatMessage(data, {
      ...
      onMessageEnd: (messageEnd) => {
        ...
        console.log('🚀 ~ handleSend ~ messageEnd:', messageEnd)
            ...
            hasSetInputs && (
              <div
                className={cn(
                  ...
                  {
                    'pb-[66px]': !APP_SHOW_MODE_SWITCH,
                    'pb-[88px]': APP_SHOW_MODE_SWITCH,
                  },
                )}>
